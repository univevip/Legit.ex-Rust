	...
	void get_list( )
	{
		while ( true )
		{
			if ( rainy.player != 0 )
			{
				...
				if (!base_entity)
				{
					std::cout << "Get Base Entity Broken" << std::endl;
					continue;
				}
				...
				if (!client_entities)
				{
					std::cout << "Client Entities Broken" << std::endl;
					continue;
				}
...
namespace features
{
	...
	auto fat_bullet( c_held *held )
	{
		...
		auto List = driver->read<uintptr_t>(0x398); //private List<Projectile> createdProjectiles;
		...
		for (int i = 0; i < size; ++i)
		{
			...
			if (driver->read<float>(Item + 0x2C) != 100.f)
			{
				driver->write<float>(Item + 0x2C, config.Includes.bulletsize);
			}
		}
	...
	auto TodSkyV2(c_tod_sky* tod_sky) -> void
	{
		...
		uintptr_t TodSkybbc1 = driver->read_chain<uintptr_t>(game_assembly, { m_tod_sky_c, 0xB8, 0x0, 0x10, 0x20 }); //56977336
		uintptr_t day = driver->read<uintptr_t>(TodSkybbc1 + 0x50);
		uintptr_t sky_color = driver->read<uintptr_t>(day + 0x28);
		uintptr_t sky_gradient = driver->read<uintptr_t>(sky_color + 0x10);
		driver->write<Color1>(sky_gradient, config.Includes.sky_colord);
		uintptr_t clouds = driver->read<uintptr_t>(TodSkybbc1 + 0x78);
		driver->write<float>(clouds + 0x14, 0);
	...
	auto night_mode( c_tod_sky *tod_sky ) -> void
	{
		...
		uintptr_t TOD_DayParameters = driver->read_chain<uintptr_t>( TOD_Sky, { 0xB8, 0x0, 0x10, 0x20, Offsets::Classes::TOD_Sky::Day } );
		uintptr_t TodSkybbc1 = driver->read_chain<uintptr_t>(game_assembly, { m_tod_sky_c, 0xB8, 0x0, 0x10, 0x20 }); //56977336
		uintptr_t SKYINSTANCE = driver->read<uintptr_t>(TOD_Sky + 0xB8);
		uintptr_t SKYINSTANCELIST = driver->read<uintptr_t>(SKYINSTANCE + 0x0);
		uintptr_t INSTANCES = driver->read<uintptr_t>(SKYINSTANCELIST + 0x10);
		uintptr_t Dome = driver->read<uintptr_t>(INSTANCES + 0x20);
		uintptr_t TodCycle = driver->read<uintptr_t>(Dome + 0x38);
		driver->write<float>(TodCycle, 85.0f);
		...
		if ( config.Includes.beautifulnight == true )
		{
			...
			//Moon Stuff	
			driver->write<float>(TOD_MoonParameters + Offsets::Classes::TOD_MoonParameters::MeshSize, 14.5f);
			driver->write<float>(TOD_MoonParameters + Offsets::Classes::TOD_MoonParameters::MeshBrightness, 10.5f);
			driver->write<float>(TOD_MoonParameters + Offsets::Classes::TOD_MoonParameters::HaloSize, 20.5f);
			driver->write<float>(TOD_MoonParameters + Offsets::Classes::TOD_MoonParameters::Position, 15.5f);
			driver->write<float>(MoonColor, 15.0f);
			driver->write<float>(MoonColor + 0x4, 5.0f);
			driver->write<float>(MoonColor + 0x8, 7.6f);
			driver->write<float>(MoonColor + 0xC, 13.6f);
			driver->write<float>(MoonColor + 0x10, 6.6f);
			driver->write<float>(MoonColor + 0x14, 2.0f);
			driver->write<float>(MoonColor + 0x18, 5.0f);
			driver->write<float>(MoonColor + 0x1C, 12.0f);
			//Star Stuff
			driver->write<float>( TOD_StarParameters + Offsets::Classes::TOD_StarParameters::Brightness, 456.f );
			driver->write<float>( TOD_StarParameters + Offsets::Classes::TOD_StarParameters::Size, 10.5f );
			//Night Stuff
			driver->write<float>( TOD_NightParameters + Offsets::Classes::TOD_NightParameters::LightIntensity, 16.f );
			driver->write<float>( TOD_NightParameters + Offsets::Classes::TOD_NightParameters::AmbientMultiplier, 20.f );
		}
	}
	...
	auto tod_sky( baseplayer *player ) -> void
	{
		...
		if ( GetAsyncKeyState(config.Includes.longneckup) ) //UP
			player->player_eyes( )->ViewOffset( Vector3( 0.0f, 1.f, 0.f ) );

		if (GetAsyncKeyState(config.Includes.longneckleft)) //LEFT
			player->player_eyes()->ViewOffset(Vector3(-1.f, 0.f, 0.f));

		if (GetAsyncKeyState(config.Includes.longneckright)) //RIGHT
			player->player_eyes()->ViewOffset(Vector3(1.f, 0.f, 0.f));
		...
		if (config.Includes.skyColor)
		{
			features::TodSkyV2(entityloop::tod_sky);
		}
	}
	auto process( baseplayer *player ) -> void
	{
		...
		if ( movement )
		{
			...
			if (config.Includes.spinbot)
			{
				auto main_offsets = driver->read<uintptr_t>(game_assembly + m_BaseEntity_c);
				uintptr_t Local_Player = driver->read_chain<uintptr_t>(main_offsets, { 0xb8, 0x20, 0x10, 0x28, 0x18, 0x20 });
				auto aPlayerInput = driver->read<uintptr_t>(Local_Player + Classes::BasePlayer::input);
				driver->write<Vector3>(aPlayerInput + Classes::BasePlayer::bodyRotation, Vector3(rand() % 99999999999 + -1999, rand() % 999999999999999 + -9999, rand() % 999999999999 + -999));
				driver->write<Vector3>(aPlayerInput + Classes::BasePlayer::headRotation, Vector3(rand() % 9999 + -9999, rand() % 99999 + -999, rand() % 9999 + -9999));
			}
		...
		if (GetAsyncKeyState(config.Includes.zoomkey) & 0x8000)
		{
			player->changefov(43.f);
		}
		else
		...
		if ( projectile && projectile->is_weapon( ) )
		{
			...
			if ( held )
			{
				...
				if ( config.Includes.reloadindicator && held->get_is_reloading( ) ){
					...
					config.Includes.m_reload_duration = held->get_reload_time() * 0.00f;
				}
				...
				if (  itemname == "pistol.eoka"&& config.Includes.instanteoka){
...
namespace visuals
{
	...
	auto chams( baseplayer *player, c_tod_sky *tod_sky)
	{
		...
		const auto rays = tod_components->get_rays();
		...
		const auto material = rays->get_material( );
					...
					if ( distance < config.Includes.playerdistance )
					{
						if ( config.Includes.esp )
						{
							...
							if ( config.Includes.name )
							{
								...
								gui->AddText( { top.x - ( ImGui::CalcTextSize( utf_8_1.c_str( ) ).x / 2 ), top.y - 14 }, FtIM(config.Includes.NameColor), utf_8_1.c_str( ) );
							}
							if ( config.Includes.boxesp )
							{
								gui->AddRect( { top.x - size.y / w , top.y }, { bottom.x + size.y / w, bottom.y + 3 }, FtIM(config.Includes.BoxColor), 0, 0, 2 );
							}
							if ( config.Includes.distance )
							{
								std::string distanceStr = "  " + std::to_string( distance );
								std::string utf_8_1 = string_To_UTF8( distanceStr );
								gui->AddText( { top.x - ( ImGui::CalcTextSize( utf_8_1.c_str( ) ).x / 2 ), top.y - 24 }, FtIM(config.Includes.distColor), utf_8_1.c_str( ) );
							}
							...
							if ( config.Includes.weapon )
							{
								...
								gui->AddText( { bottom.x - ( ImGui::CalcTextSize( utf_8_1.c_str( ) ).x / 2 ), bottom.y + 12 }, FtIM(config.Includes.WeaponColor), utf_8_1.c_str( ) );
							}
							...
							{
								gui->AddRectFilled( { top.x - size.y / w , top.y }, { bottom.x + size.y / w, bottom.y + 3 }, FtIM(config.Includes.BoxfilledColor));
			...
			if ( config.Includes.prefabesp )
			{
				...
				if ( entityloop::camera->WorldToScreen( &entityloop::projection_matrix, std::get<0>( prefab ), &position_2d ) )
				{
					if ( selected )
					{
						...
						std::string distanceStr = "   " + std::to_string( (int)distance );
		...
		while ( true )
		{
			for ( auto &ply : rainy.player_list )
			{
				...
				if ( features::AimFov( ply ) < config.Includes.aimfov )
				{
					...
					{
						if ( GetAsyncKeyState(config.Includes.aimbotkey) )
						{
